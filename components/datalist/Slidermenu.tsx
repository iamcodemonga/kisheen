...
const Slidermenu = async({ meals, allowcart, discount }: { meals: TMeal[], allowcart: boolean, discount: number }) => {
        ...
        <section>
            <div className='lg:flex mt-14 lg:mt-36 w-full'>
                ...
                <div className='slider-container flex gap-3 lg:gap-6 overflow-x-auto w-full px-4 relative pb-3 scroll-smooth'>
                    {meals.length > 0 ? meals.map((meal: TMeal, index: number) => <div className='slider-card relative' key={meal.id}>
                        ...
                        <h4 className='mb-1 mt-5 text-lg md:text-2xl font-normal'><Link href={meal.slug}>{meal.title}</Link></h4>
                        ...
                        {discount > 1 ?  <span className='px-2 pt-2 pb-1 bg-green-700 absolute top-0 mt-2 rounded-lg left-0 ml-2 text-xs text-primary font-normal'>{discount}% off
                        </span> : null}
                        {!meal.quantity ? <span className='px-4 pt-2 pb-1 bg-red-500 absolute top-0 mt-0 right-0 -mr-3 text-sm text-primary font-black uppercase rotate-12'>Sold out!</span> : <button className={`px-6 py-2 bg-white/60 backdrop-blur-md text-primary rounded-lg hover:bg-accent ${allowcart ? "flex" : "hidden"} items-center justify-start absolute top-0 right-0 mt-2 mr-2`} type='button' onClick={() => handleAddToCart({id: meal.id, photo: meal.photo.url, name: meal.name, title: meal.title, slug: meal.slug, price: meal.price, quantity: meal.quantity, cartQty: 1, type: meal.type, category: meal.category, combo: meal.combo?.split(',')[0], comboList: meal.combo?.split(','), meat: meal.meat?.split(',')[0], meatList: meal.meat?.split(',')})}>
