...
import { AllMeals, CategorisedMeals, FilterdedMeals, SearchMeals } from '@/lib/graphcms'
...
const Menusection = ({ meals, active, initialMinimumPrice, initialMaximumPrice, discount, allowcart }: Props) => {
          ...
          <section className='container py-6 lg:py-16 col-span-6 lg:col-span-5'>
              ...
              <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-8 lg:gap-y-14'>
                  {!loading.page ? food ? food.slice(0, limit).map((meal:TMeal) => <div className='card w-full relative' key={meal.id}>
                      ...
                      <h4 className='mt-4 mb-1 text-lg lg:text-lg font-normal'><Link href={meal.quantity ? `/meal/${meal.slug}` : ``}>{meal.title}</Link></h4>
                      ...
                      {discount > 0 ? <span className='px-2 pt-2 pb-1 bg-green-700 absolute top-0 mt-2 rounded-lg left-0 ml-2 text-xs text-primary font-medium'>{discount}% off</span> : null}
                      {meal.quantity ? <button className={`px-6 py-2 bg-white/60 backdrop-blur-md text-primary rounded-lg hover:bg-accent ${allowcart ? "flex" : "hidden"} items-center justify-start absolute top-0 right-0 mt-2 mr-2`} type='button' onClick={() => handleAddToCart({id: meal.id, photo: meal.photo.url, name: meal.name, title: meal.title, slug: meal.slug, price: meal.price, quantity: meal.quantity, cartQty: 1, type: meal.type, category: meal.category, combo: meal.combo?.split(',')[0], comboList: meal.combo?.split(','), meat: meal.meat?.split(',')[0], meatList: meal.meat?.split(',')})}>
                  ...
                  </div>): <EmptyPosts illustration='/searchnotfound.svg' heading="Search not found!" cta="Back to menu" />: <MockPostGrid list={[1, 2, 3, 4, 5, 6]} />}
                  {loading.more ? <MockPostGrid list={[1, 2, 3]} /> : null}
              </div>
              {food.length < 1 && <EmptyPosts illustration='/searchnotfound.svg' heading="Search not found!" cta="Back to menu" />}
              <p className='text-center mt-10 md:mt-14 lg:mt-16'>
                {limit <= food.length ? <button type='button' className='px-8 py-3 text-base bg-gray-900 font-normal rounded-full text-primary' onClick={() => handleMore(Number(minPrice), Number(maxPrice), activeRoute )}>Load more</button> : null}
              </p>
          </section>
      </section>
  )
